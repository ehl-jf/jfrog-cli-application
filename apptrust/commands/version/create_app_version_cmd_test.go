...
// --- printCreateAppVersionResponse tests ---

const sampleCreateAppVersionJSON = `{"application_key":"my-app","version":"1.0.0","status":"COMPLETED","current_stage":"dev","tag":"release-tag"}`

func TestPrintCreateAppVersionResponse_JSON(t *testing.T) {
	var buf bytes.Buffer
	err := printCreateAppVersionResponse([]byte(sampleCreateAppVersionJSON), coreformat.Json, &buf)
	assert.NoError(t, err)
	// The JSON path goes through log.Output, not the writer — assert no error and valid JSON.
	var parsed map[string]interface{}
	assert.NoError(t, json.Unmarshal([]byte(sampleCreateAppVersionJSON), &parsed))
}

func TestPrintCreateAppVersionResponse_Table(t *testing.T) {
	var buf bytes.Buffer
	err := printCreateAppVersionResponse([]byte(sampleCreateAppVersionJSON), coreformat.Table, &buf)
	assert.NoError(t, err)
	output := buf.String()
...
func TestPrintCreateAppVersionResponse_Table_AbsentFieldsOmitted(t *testing.T) {
	...
	var buf bytes.Buffer
	err := printCreateAppVersionResponse([]byte(payload), coreformat.Table, &buf)
	assert.NoError(t, err)
	output := buf.String()
	assert.Contains(t, output, "application_key")
	assert.Contains(t, output, "my-app")
...
func TestPrintCreateAppVersionResponse_None_BackwardCompat(t *testing.T) {
	// When outputFormat is None (no flag set), the function must not error.
	var buf bytes.Buffer
	err := printCreateAppVersionResponse([]byte(sampleCreateAppVersionJSON), coreformat.None, &buf)
	assert.NoError(t, err)
}

func TestPrintCreateAppVersionResponse_Table_InvalidJSON(t *testing.T) {
	var buf bytes.Buffer
	err := printCreateAppVersionResponse([]byte("not-json"), coreformat.Table, &buf)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "failed to parse version response")
}
